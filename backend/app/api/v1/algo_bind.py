...
async def bind_algorithm(req: BindRequest):
    ...
    logger.info("Received bind request: bindId=%s, cameraId=%s, algorithms=%s", req.bindId, req.cameraId, req.algorithmList)
    ...
    try:
        logger.info("Successfully bound camera %s with bindId %s to algorithms %s", req.cameraId, req.bindId, req.algorithmList)
        await _monitor_service.init_single_stream(stream_cfg)
    except Exception as e:
        logger.error("Failed to bind camera %s with bindId %s: %s", req.cameraId, req.bindId, str(e))   
